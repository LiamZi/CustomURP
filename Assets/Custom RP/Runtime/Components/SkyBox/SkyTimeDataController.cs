...
namespace CustomURP
{
    ...
    public class SkyTimeDataController : MonoBehaviour
    {
        ...
        public Texture2D GenerateSkyGradientColorTexture(Gradient start, Gradient end, int resolution, float lerpVal)
        {
            ...
            {
                Color low = start.Evaluate(i * 1.0f / resolution).linear;
                Color up = end.Evaluate(i * 1.0f / resolution).linear;

                Color fin = Color.Lerp(low, up, lerpVal);
